...
mod2 = glm(vs ~ hp, data = df, family = "binomial") #generalize linearmodel #family of fuctions #binomial is good for cathegorical true and false 
df$binom.pred <- predict(mod2, type = "response")#allows to keep using the original values 
...
df$binom.pred <- predict(mod3, type = "response")#allows to keep using the original values 
