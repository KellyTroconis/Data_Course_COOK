...
```{r, echo=TRUE}
?read.csv() #This brings up the help file
...
**Questions:**
  ...
  You can save you data as a data frame. It lest R now that the data is coma separated 
  
  + **2. How is it different from read.csv2()?**
  For csv2 the commas are decimnals and the periods is the separation 
  
  + **3. Why does read.csv2() even exist?**
  Other countries numeric system where decimals are semarated by commas not periods. 
...
summary(df$Home.Value)
  ...
  [1] "Home.Value" 
  #because Home.Value is the media in the fourth colum 
____________
...
**Questions:**
  ...
  **In other words, what happens when you run: plot(x=df\$Year,y=df\$Land.Value,col=df\$region)**
  the data points are color by region 
