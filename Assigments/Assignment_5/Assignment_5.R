...
ggplot(midwest) # what do you see? #clean set up 
...
# Zoom in
p + lims(x=c(0,0.1),y=c(0,1000000)) # what did this do? # changes Xs and Ys values
p + coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) # how is this different? it changes the fit line
...
#geom_point(color="steelblue",size=3) + ..... changed the color and size of the dots 
#geom_smooth(method="lm",color="firebrick")......changed the color of the line 
#this two lines of coding are different from the original 
...
p3 <- ggplot(midwest, aes(x=area, y=poptotal)) + 
  ...
  coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) + 
  labs(title="Area Vs Population", subtitle="From midwest dataset", y="Population", x="Area", caption="Midwest Demographics")
p3 #this creates a new data frame wich later you can modified just the color if wanted with just one line of coding as... p3 + scale_color_brewer(palette = "Set1") 

# Don't like those colors?
p3 + scale_color_brewer(palette = "PuOr")
